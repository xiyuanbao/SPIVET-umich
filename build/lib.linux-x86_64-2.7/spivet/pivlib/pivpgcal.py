...
def ccpts(imsin,rbndx,knowns,ccptune,ofpath=None,show=True):
    ...
    for i in range(len(imsin)):
        ...
        if ( ( i == 0 ) and ( show ) ):
            pylab.figure()
            pylab.imshow(mim,cmap=pylab.cm.gray,interpolation='nearest')
            pylab.setp(pylab.gca(),xticks=[],yticks=[])
            pylab.title("Input Image after Specified Preprocessing")
        ...
        if ( ( ( i == 0 ) and ( show ) ) or ( cmsng.size > 0 ) ):
            ...
            pylab.setp(pylab.gca(),xticks=[],yticks=[])
            pylab.title("Intersections" +tstr)

            dni = 1
            if ( ni > 200 ):
                dni = 10
            for i in range(0,ni,dni):
                pylab.text(tc[i,1],tc[i,0],str(i),color='w',size=6)
